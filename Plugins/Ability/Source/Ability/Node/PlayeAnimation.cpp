...
void UPlayeAnimation::OnActiveNode()
{
	...
	if (auto MobaAbilityComponent = GetOwnerPawn()->FindComponentByClass<UMobaAbilityComponent>())
	{
		MobaAbilityComponent->PlayAnimation(Asset, Tags);
	}
	else
	{
		UE_LOG(Ability, Error, TEXT("[%s] [%s] : Owner not MobaAbilityComponent"), *Ability_LINE, GetFName());
	}
}
